...
class Scene extends Group {
  ...
  clientToWorld(clientX: number, clientY: number) {
		return this.clipToWorld(this.clientToClip(clientX,clientY))
	}

	/* 基于某个坐标系坐标系，判断某个点是否在图形内 */
	/* isPointInObj(obj: Object2D, mp: Vector2, matrix: Matrix3 = obj.pvmMatrix) {
		ctx.save()
		obj.crtPath(ctx, matrix)
		bool = ctx.isPointInPath(mp.x, mp.y)
		ctx.restore()
		return bool
	} */
}
